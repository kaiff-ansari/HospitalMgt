...
@router.post("/add")
def add_blood_inventory(blood:BloodInventoryCreate, db:Session = Depends(get_db)):
...
def update_inventory(inventory_id: int, blood: BloodInventoryCreate, db: Session = Depends(get_db)):
